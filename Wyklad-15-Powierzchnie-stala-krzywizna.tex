...
\pause\begin{center}
\input{./pictures/pseudosphere.tex}
\end{center}
...
% \begin{align*}
% \frac{d^2\le[v(u(t))\ri]}{d u^2(t)}&= \frac{d}{du(t)}\le(\frac{v'(u(t)}{u'(t)}\ri)=\\
% &=\frac{u'(t)v''(u(t))-v'(u(t))u''(t)}{\le(u'(t)\ri)^2}\cdot\frac{1}{u'(t)}.\end{align*}
% Podstawmy teraz drugie pochodne wyliczone z równań geodezyjnych
% \[=\frac{1}{\le(u'(t)\ri)^3}\le[ u'(t)\le(\frac{1}{v(t)}\le((v'(t))^2-(u'(t))^2\ri)\ri) - v'(t)\le( \frac{2}{v(t)}u'(t)v'(t)\ri)\ri],\]
% i uprośćmy to wyrażenie:
% \begin{align*}
% &=\frac{u'(t)}{\le(u'(t)\ri)^3}\cdot\frac{1}{v(t)}\le[\le((v'(t))^2-(u'(t))^2\ri)-2(v'(t))^2\ri]=\\
% &=-\frac{1}{v(t)}\le[1+\le(\frac{v'(t)}{u'(t)}\ri)^2\ri]=\frac{-1}{v(t)}\le(1+\le(\frac{dv(u(t)}{du(t)}\ri)^2\ri).
% \end{align*}
\begin{align*}
...
\begin{frame}
\begin{center}
\input{./pictures/hyperb-ratio.tex}
\end{center}
...
\begin{frame}
\mode<presentation>{
Jak przekonamy się za chwilę nie istnieje gładkie zanurzenie \textit{płaskiego} torusa w $\R^3$ (tj. o stałej krzywiźnie równej $0$). Możemy jednak w łatwy sposób wskazać płaskie zanurzenie $T^2$w przestrzeń $\R^4$. Na początek przypomnijmy, że torus może być utożsamiony z $S^1\times S^1$. } \pause Rozważmy kwadrat $U=[0,2\pi)\times [0,2\pi)\subset \R^2$. Niech $x\colon U\to \R^3$ będzie określone wzorem

\mode<presentation>{\begin{tabular}{m{2.2in} m{2in}}}
\mode<article>{\begin{tabular}{m{3in} m{2in}}}
$x(u,v)= (\cos u, \sin u,\cos v,\sin v)$& $ \text{\input{./pictures/flat-torus-square.tex}}$
\end{tabular}
...
\input{./pictures/octagon-2-torus.tex}
...
\begin{center}
\input{./pictures/hyperb-octagon.tex}
\end{center}
...
\begin{center}
\input{./pictures/torus-rotation-non-equal-paths.tex}
\end{center}
...
\begin{frame}
\begin{center}
\input{./pictures/torus-inside-sphere.tex}
\end{center}
...
\begin{frame}[plain]
\mode<presentation>{\hspace*{-0.9in}\includegraphics[scale=2]{./pictures/C1-flat-torus-1.png}}
\mode<article>{\includegraphics[keepaspectratio=true,width=\textwidth]{./pictures/C1-flat-torus-1.png}}
% \begin{center}
...
%\newpage
\begin{frame}[plain]
\mode<presentation>{\hspace*{-0.9in}\includegraphics[scale=2]{./pictures/C1-flat-torus-2.png}}
\mode<article>{\includegraphics[keepaspectratio=true,width=\textwidth]{./pictures/C1-flat-torus-2.png}}
\end{frame}
...
\begin{center}
\mode<presentation>{\includegraphics[scale=0.41]{./pictures/C1-flat-torus-equal.jpg}}
\mode<article>{\includegraphics[keepaspectratio=true,width=\textwidth]{./pictures/C1-flat-torus-equal.jpg}}
...
\begin{frame}[plain]
%\begin{center}
\mode<presentation>{\hspace*{-0.2in}\includegraphics[scale=1.5]{./pictures/flat-torus-1.pdf}}
\mode<article>{\hspace*{0.4in}\includegraphics[keepaspectratio=true,width=\textwidth]{./pictures/flat-torus-1.pdf}}

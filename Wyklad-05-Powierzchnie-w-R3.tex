...
\input{./pictures/sphere-atlas.tex}
%\end{tikzpicture}
\end{center}
\end{przyklad}
...
\input{./pictures/transition-function.tex}
%\end{tikzpicture}
\end{center}
...
\begin{center}
%\begin{tikzpicture}[y=0.80pt, x=0.8pt,scale=0.4,yscale=-1, inner sep=0pt, outer sep=0pt]
\input{./pictures/smth-func-on-mfld.tex}
%\end{tikzpicture}
\end{center}
...
\end{definicja}
\begin{itemize}
\item \mode<article>{[\textbf{Uwaga:} ]}W naszym przypadku oznacza to, że wszystkie pochodne cząstkowe są równe $0$.
\mode<article>{\item [\textbf{Uwaga 2:} ]W przypadku wyżej-wymiarowym, dla odwzorowania $f\colon \R^m\to \R^n$ warunek $\text{rank }DF(p)=0$   powinien być zastąpiony przez: \[\text{rank }DF(p) \text{ jest mniejszy od maksymalnego, tj. }DF(p)<\max(m,n).\]}
...
\item \mode<article>{[\textbf{Uwaga:}]}W naszym przypadku oznacza to, że przynajmniej jedna z pochodnych cząstkowych odwzorowania $F$ jest różna od $0$ w tym punkcie.
\mode<article>{\item [\textbf{Uwaga 2:}]Warunek $\text{rank }DF(p)=1$ w definicji punktu regularnego tak naprawdę oznacza: rząd tak duży jak tylko jest to możliwe. Jeśli funkcja $F$ będzie miała wartości w $\R^n$ definicję trzeba będzie odpowiednio zmodyfikować. }

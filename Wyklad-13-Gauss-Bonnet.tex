...
\begin{frame}
Na podstawie powyższego lematu nie możemy wnioskować, czy zbiór $E_p$ jest otwarty w $T_pM$, ani czy ma niepuste wnętrze. Chociaż nie jest to oczywiste $E_p$ zawiera kulę otwartą o odpowiednio małym promieniu. \mode<article>{Co więcej, zachodzi następujący lemat.}
\pause \begin{lemat}
...
Przypomnijmy, że przez $D(T_pM,\delta_p)$ oznaczamy dysk o środku w punkcie $\mathcal{O}=(0,0)\in T_pM$ i promieniu $\delta_p$.
...
\mode<article>{Jeśli $M$ jest powierzchnią zamkniętą (tj. zwartą i bez brzegu), wtedy korzystając ze zwartości można wykazać, że istnieje jedna uniwersalna stała $\delta_M$ zależna tylko od $M$, którą można podstawić w miejsce $\delta_p$ w powyższych trzech stwierdzeniach.}
